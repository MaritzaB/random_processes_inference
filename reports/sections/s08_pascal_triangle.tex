...
\addcontentsline{toc}{subsection}{Suma de las filas}
...
\addcontentsline{toc}{subsection}{N\'umeros triangulares}
...
\begin{figure}[h!]
	...
	\foreach \i in {0,...,\N}{
		\foreach \j in {0,...,\i}{
			\def\val{\pasc{\i}{\j}};
			\ifthenelse{\j=2}{\def\mycolor{gray}}{\def\mycolor{white}};
			\draw[fill = \mycolor!50] ({-0.5*\i+\j},-\i) circle (0.5);
			\node at ({-0.5*\i+\j-0.2},-\i){\val};
		}
	}
	\end{tikzpicture}
\end{figure}
...
\begin{figure}[h!]
	...
	\foreach \i in {0,...,\N}{
		\foreach \j in {0,...,\i}{
			\def\val{\pasc{\i}{\j}};
			\ifthenelse{\j=1}{\def\mycolor{gray}}{\def\mycolor{white}};
			\draw[fill = \mycolor!50] ({-0.5*\i+\j},-\i) circle (0.5);
			\node at ({-0.5*\i+\j-0.2},-\i){\val};
		}
	}
